...
expect.extend({
  ...
  toHaveBeenCalledTimes(received: unknown, n: number): MatchResult {
    ...
    return built(
      spy.callCount === n,
      `expected mock to have been called ${n} times (actual: ${spy.callCount})`,
  ...
  toHaveBeenNthCalledWith(received: unknown, nth: number, ...expected: unknown[]): MatchResult {
    const spy: MethodSpy = asSpy(received)
    return built(
      argsMatchAt(spy, nth - 1, expected),
      `expected mock's call #${nth} to match the given args`,
      `expected mock's call #${nth} not to match the given args`
    )
  },
})
