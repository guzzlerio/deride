...
|------------|--------------------|
| Create test doubles from interfaces, instances, classes, or standalone functions | [`stub` / `wrap` / `func`](/guide/creating-mocks) |
| Configure behaviour (return, resolve, throw, yield, sequence…) | [`setup.*`](/guide/configuring-behaviour) |
| Assert how the mock was called | [`expect.*.called`](/guide/expectations) |
| Inspect call history without throwing | [`spy.*`](/guide/spy) |
| Match arguments expressively | [`match.*`](/guide/matchers) |
| Verify cross-mock call order | [`inOrder(...)`](/guide/ordering) |
| Group mocks, snapshot, restore | [`sandbox` / `snapshot`](/guide/lifecycle) |
...
Next up: [installation](/guide/installation), then a [5-minute quick start](/guide/quick-start).
