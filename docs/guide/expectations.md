...
Reset leaves behaviours (from `setup`) intact. To wipe both behaviours and history, use `mock.restore(mock.snapshot())` at an earlier "clean" state, or see [Lifecycle](/guide/lifecycle#snapshot-restore).
...
This is true for `withArg`, `withArgs`, `withReturn`, and the `everyCall.*` variants. See [Diagnostics](/guide/diagnostics) for more on improving failure output.
