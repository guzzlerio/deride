...
`stub.class` produces a newable stand-in, but you still need to substitute it where the real constructor is imported. That means `vi.mock('./database', () => ({ Database: MockedDb }))` (or jest's equivalent). See [Module mocking](./module-mocking).
:::
...
- [Module mocking](./module-mocking) — substituting mock classes at import time
